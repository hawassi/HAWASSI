...
%%%%%%%%%%%%%%%%%  RHS for RunUp  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dz = RHSShore0(time,z,model,par,influx,IVP,Oprt,ProgStatbar)
%%%%%passing parameter%%%%%%%%%%%%%%
...
IntCoefCder =par.interp.IntCoefCder;
...
elseif model.nonlinear==2 
    ...
    gmCder=zeros(n,1);gpCder=gm;gcCder=gp;
    if FlagOvertop==0
    ...
    gmCder(indWI:indWF)=ppval(IntCoefCder.m,H(indWI:indWF));
    gpCder(indWI:indWF)=ppval(IntCoefCder.p,H(indWI:indWF));
    gcCder(indWI:indWF)=ppval(IntCoefCder.c,H(indWI:indWF));
    else
    ...
    Cu                    = Ifft(Cu_hat);
    Cu(H<H_min)            = sqrt(g.*H(H<H_min)).*u(H<H_min);
    Cu_ShoreChar          = Ifft(Cu_hat).*ShoreChar.*Wallchar;
    Cu_ShoreChar_hat      = fft(Cu_ShoreChar);
    ConjC_Cu_ShoreChar_hat= Op_C(Cm,gm,Cp,gp,Cc,gc,...
    Cu_ShoreChar,Cu_ShoreChar_hat);
    ConjC_Cu_ShoreChar=Ifft(ConjC_Cu_ShoreChar_hat);
    ConjC_Cu_ShoreChar(H<H_min)=sqrt(g.*H(H<H_min)).*Cu_ShoreChar(H<H_min);
    ConjC_Cu_ShoreChar_hat=fft(ConjC_Cu_ShoreChar);
    Cder_u_hat             = Op_C(Cder_m,gmCder,Cder_p,gpCder,Cder_c,gcCder,u,z2);
    Cder_u                = Ifft(Cder_u_hat);
    Cder_uCu              =Cder_u.*Cu;
    Cder_uCu(H<H_min)     =g*u(H<H_min).^2/2;
    deluH_hat   =  1/g.*ConjC_Cu_ShoreChar_hat;
    deletaH_hat =  g.*z1+ fft(Ifft(fft(Cder_uCu)./g).*Chi.*Wallchar);
...
elseif model.nonlinear>2  
    ...
    if model.nonlinear==2
        ...
        M1etaM0dxetaM0uWallChar_hat= Op_M13p(k,g,Cm2,gm,Cp2,gp,Cc2,gc,etaM0dxetaM0uChar_hat);
