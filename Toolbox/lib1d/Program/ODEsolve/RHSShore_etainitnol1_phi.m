...
%%%%%%%%%%%%%%%%%  RHS for RunUp  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dz = RHSShore_etainitnol1_phi(time,z,model,par,influx,IVP,Oprt,ProgStatbar)
%%%%%passing parameter%%%%%%%%%%%%%%
...
elseif model.nonlinear>=2
    ...
    C_u_hat         = Op_Cr(Cm1,gm1,Cp1,gp1,Cc1,gc1,Cm2,gm2,Cp2,gp2,Cc2,gc2,u,uhat);
...
%     end
    ...
    C2u_hat          = Op_C2r(C2m1,gm1,C2p1,gp1,C2c1,gc1,C2m2,gm2,C2p2,gp2,C2c2,gc2,u,uhat);
...
if strcmp(model.breaking.check,'Yes')
    ...
    Rb_hat=fft(oneperH.*Flux.*Wallchar);
    %         if time>30
    %         subplot(4,1,2)
    %         Rb=Ifft(Rbhat);
...
dtphi_hat   = (dtphi_hat+Rb_hat+Rf_hat+Rw_hat-fft(par.dampcoef.*Upu.*dampchar)./(1i.*k)).*aal;
...
if par.IP.check==1
    if time>=par.IP.time(1) && time<=par.IP.time(2)
        if time>=par.IP.time(1)+(iterInteriordt-1)*par.dt*par.IP.time(3)
            ...
            dtphihat(iterInterior,:)=dz(n+1:end);
