...
%%%%%%%%%%%%%%%%%  RHS for RunUp  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dz = RHSShore_etainitnol2(time,z,model,par,influx,IVP,Oprt,ProgStatbar)
%%%%%passing parameter%%%%%%%%%%%%%%
...
%IndS0=par.IndS;
%eta(x>x(IndS0))=eta(IndS0)-par.bathyplus(x>x(IndS0));
...
% %    uChar_hat        =fft(uChar);
% %     indH=find(H<H_minDispInterp & H>H_minShore);
% %     C2op_exact_mat=UpExact(k,H(indH)).^2;
% %     uChar_hat_mat   =repmat(uChar_hat,1,length(indH));
% %     C2uChar_hat_mat =C2op_exact_mat.*uChar_hat_mat;
% %     C2uChar_mat     =Ifft(C2uChar_hat_mat);
% %     CCu_ShoreChar(indH)=diag(C2uChar_mat);
% %    CCu_ShoreChar(H<=H_minShore) = g*H(H<=H_minShore).*uChar(H<=H_minShore);
% 
