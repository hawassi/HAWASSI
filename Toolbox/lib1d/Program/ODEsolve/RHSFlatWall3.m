...
function [dz] = RHSFlatWall3(time,z,model,par,influx,IVP,Oprt,ProgStatbar)
%%%%%passing parameter%%%%%%%%%%%%%%
...
Fskewwall=cumtrapz(influx.timesig_insig(:,1),Sws);
skewFwall=interp1(influx.timesig_insig(:,1),Fskewwall,time);

% assignin('base','Sws',Sws);
%assignin('base','Swall_tS',Swall_tS);
% assignin('base','Sw_skew',Sw_skew);

%skewFwall=Sw_skew(IndtInterp);%interp1(influx.timesig_insig(:,1),Sw_skew,time);
