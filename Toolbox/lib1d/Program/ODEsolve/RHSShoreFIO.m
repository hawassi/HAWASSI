...
%%%%%%%%%%%%%%%%%  RHS for RunUp  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dz = RHSShoreFIO(time,z,model,par,influx,IVP,Oprt,ProgStatbar)
%%%%%passing parameter%%%%%%%%%%%%%%
...
%     CCu_ShoreChar    =Ifft(CCu_hat).*ShoreChar.*Wallchar;
    ...
    uChar_hat        =fft(uChar);
    Cop_exact_mat   =UpExact(k,H);
    uChar_hat_mat    =repmat(uChar_hat,1,length(H));
    CuChar_hat_mat  =Cop_exact_mat.*uChar_hat_mat;
    Cu_ShoreChar_hat=fft(diag(Ifft(CuChar_hat_mat))+Ifft(diag(CuChar_hat_mat)))/2;
    Cu_ShoreChar=Ifft(Cu_ShoreChar_hat);
    Cu_ShoreChar   =repmat(Cu_ShoreChar,1,length(H));
    CadjCu_ShoreChar=Cop_exact_mat.'.*Cu_ShoreChar;
    CCu_ShoreChar_hat =(fft(diag(CadjCu_ShoreChar))+diag(fft(CadjCu_ShoreChar)))/2;
