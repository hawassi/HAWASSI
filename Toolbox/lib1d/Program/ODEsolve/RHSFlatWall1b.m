...
function [dz] = RHSFlatWall1b(time,z,model,par,influx,IVP,Oprt,ProgStatbar)
%%%%%passing parameter%%%%%%%%%%%%%%
...
dxWallchar  =par.wall.dxWallchar';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
L0phi_hat         =Oprt.M0.*z2;  L0phi=Ifft(L0phi_hat);
delphiH1_hat     = fft(L0phi.*Wallchar)-fft(Ifft(1i.*k.*Cp.*fft(Wallchar)).*Ifft(Cp.*z2)./g);
deletaH1_hat    =  g.*1i.*k.*fft(eta.*Wallchar);


dteta_hat   = (delphiH1_hat + Source - fft(par.dampcoef.*eta.*dampchar)).*aal;
dtu_hat     = (-deletaH1_hat- fft(par.dampcoef.*u.*dampchar)).*aal;
