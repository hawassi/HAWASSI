...
function [Amat,Bvect] =fun_construct_matrices_diffraction_Ndom(Ndom,psibdy,dxpsibdy,tauGbdy,dxtauGbdy,Dbdy,dxDbdy,DZetabdy,dxDZetabdy,dr,Evmodes,kappaS,etaWl,dxetaWl,phiWl,dxphiWl,Xbdy)
Ns=1;
...
for n=0:Evmodes
    ...
    if n==0
    Amat(Nm*Ns*n+1,1:2)        =[ -IVp0Vpn1L , -IVp0Vpn1L*tauGbdy(1)];
    Amat(Nm*Ns*n+Nm/2,Nm-3:Nm-2) =[ IVp0Vpn1R , IVp0Vpn1R*tauGbdy(end) ];
    idS0=1;idS1=0;
    else
    Amat(Nm*Ns*n+1-2,1:2)        =[  -IVp0Vpn1L , -IVp0Vpn1L*tauGbdy(1)];
    Amat(Nm*Ns*n+Nm/2-2,Nm-3:Nm-2) =[ IVp0Vpn1R , IVp0Vpn1R*tauGbdy(end)  ];
    Amat(Nm*Ns*n+Nm/2+1-2,1:2)   =[  0          , -IQnVp01L*dxtauGbdy(1)];
    Amat(Nm*Ns*n+Nm-2,Nm-3:Nm-2)   =[ 0           ,  IQnVp01R*dxtauGbdy(end)];
    idS0=2;idS1=2;
    end
    idcol=1;IdIn=1;
        ...
        for ii=2:Nm/2-1
            if ii<(Nm/2-1)/2+1
                Amat(Nm*Ns*n+ii-idS1,idcol:idcol+3) =[IVp0VpnIn(IdIn+1), IVp0VpnIn(IdIn+1)*tauGbdy(ii), -IVp0VpnIn(IdIn+1)    , -IVp0VpnIn(IdIn+1)*tauGbdy(ii)];
                Amat(Nm*Ns*n+Nm/2+ii-idS0,idcol:idcol+3) =[ 0   , IQnVp0In(IdIn)*dxtauGbdy(ii),  0 , -IQnVp0In(IdIn+1)*dxtauGbdy(ii)];
                
            elseif ii>(Nm/2-1)/2+1
                Amat(Nm*Ns*n+ii-idS1,idcol:idcol+3) =[IVp0VpnIn(IdIn-1), IVp0VpnIn(IdIn-1)*tauGbdy(ii), -IVp0VpnIn(IdIn-1)    , -IVp0VpnIn(IdIn-1)*tauGbdy(ii)];
                Amat(Nm*Ns*n+Nm/2+ii-idS0,idcol:idcol+3) =[ 0   , IQnVp0In(IdIn-1)*dxtauGbdy(ii),  0 , -IQnVp0In(IdIn)*dxtauGbdy(ii)];
                
            else
                Amat(Nm*Ns*n+ii-idS1,idcol:idcol+3)=[IVp0VpnIn(IdIn), IVp0VpnIn(IdIn)*tauGbdy(ii), -IVp0VpnIn(IdIn)    , -IVp0VpnIn(IdIn)*tauGbdy(ii)];
                Amat(Nm*Ns*n+Nm/2+ii-idS0,idcol:idcol+3) =[ 0   , IQnVp0In(IdIn)*dxtauGbdy(ii),  0 , -IQnVp0In(IdIn)*dxtauGbdy(ii)];
    ...
    if Evmodes>0
        for m=1:Evmodes
            ...
            if n>0
                Amat(Nm*Ns*n+1-2,Nm*Ns*m+(1:3)-2) =[IQmVpn1L , -IVpmVpn1L, -exp(-muS1L_m*(Xbdy(1)-Xbdy(2)))*IVpmVpn1L ];
                Amat(Nm*Ns*n+Nm/2-2,Nm*Ns*(m+1)-4:Nm*Ns*(m+1)-2)  =[exp(muS1R_m*(Xbdy(end)-Xbdy(end-1)))*IVpmVpn1R, IVpmVpn1R,-IQmVpn1R ];
                
                Amat(Nm*Ns*n+Nm/2+1-2,Nm*Ns*m+(1:3)-2)=[kappaS(m+1,1)*IQmQn1L+IdxQmQn1L , -(muS1L_m*IQnVpm1L+IdxVpmQn1L)   , -exp(-muS1L_m*(Xbdy(1)-Xbdy(2)))*((-muS1L_m-dxmuS1L_m*(Xbdy(1)-Xbdy(2)))*IQnVpm1L+IdxVpmQn1L)];
                Amat(Nm*Ns*n+Nm-2,Nm*Ns*(m+1)-4:Nm*Ns*(m+1)-2)   =[ exp(muS1R_m*(Xbdy(end)-Xbdy(end-1)))*((muS1R_m+dxmuS1R_m*(Xbdy(end)-Xbdy(end-1)))*IQnVpm1R+IdxVpmQn1R),(-muS1R_m*IQnVpm1R+IdxVpmQn1R),kappaS(m+1,2)*IQmQn1R-IdxQmQn1R  ];
                idS0=2;idS1=2;
            else
              
                Amat(Nm*Ns*n+1,Nm*Ns*m+(1:3)-2) =[IQmVpn1L , -IVpmVpn1L, -exp(-muS1L_m*(Xbdy(1)-Xbdy(2)))*IVpmVpn1L ];
                Amat(Nm*Ns*n+Nm/2,Nm*Ns*(m+1)-4:Nm*Ns*(m+1)-2)  =[exp(muS1R_m*(Xbdy(end)-Xbdy(end-1)))*IVpmVpn1R, IVpmVpn1R,-IQmVpn1R ];
                idS0=1;idS1=0;
            end
            ...
            for ii=2:Nm/2-1
                % ii
                if ii<(Nm/2-1)/2+1
                    Amat(Nm*Ns*n+ii-idS1,Nm*Ns*m+(idcol:idcol+3)-2) =[exp(muSIn_m(IdIn+1)*(Xbdy(ii)-Xbdy(ii-1)))*IVpmVpnIn(IdIn+1), IVpmVpnIn(IdIn+1), -IVpmVpnIn(IdIn+1)    , -exp(-muSIn_m(IdIn+1)*(Xbdy(ii)-Xbdy(ii+1)))*IVpmVpnIn(IdIn+1)];
                    Amat(Nm*Ns*n+Nm/2+ii-idS0,Nm*Ns*m+(idcol:idcol+3)-2) =[ exp(muSIn_m(IdIn)*(Xbdy(ii)-Xbdy(ii-1)))*((muSIn_m(IdIn)+dxmuS1In_m(IdIn)*(Xbdy(ii)-Xbdy(ii-1)))*IQnVpmIn(IdIn)+IdxVpmQnIn(IdIn)),  -muSIn_m(IdIn)*IQnVpmIn(IdIn)+IdxVpmQnIn(IdIn) , -muSIn_m(IdIn+1)*IQnVpmIn(IdIn+1)-IdxVpmQnIn(IdIn+1), -exp(-muSIn_m(IdIn+1)*(Xbdy(ii)-Xbdy(ii+1)))*((-muSIn_m(IdIn+1)-dxmuS1In_m(IdIn+1)*(Xbdy(ii)-Xbdy(ii+1)))*IQnVpmIn(IdIn+1)+IdxVpmQnIn(IdIn+1))];
                elseif ii>(Nm/2-1)/2+1
                    Amat(Nm*Ns*n+ii-idS1,Nm*Ns*m+(idcol:idcol+3)-2) =[exp(muSIn_m(IdIn-1)*(Xbdy(ii)-Xbdy(ii-1)))*IVpmVpnIn(IdIn-1), IVpmVpnIn(IdIn-1), -IVpmVpnIn(IdIn-1)    , -exp(-muSIn_m(IdIn-1)*(Xbdy(ii)-Xbdy(ii+1)))*IVpmVpnIn(IdIn-1)];
                    Amat(Nm*Ns*n+Nm/2+ii-idS0,Nm*Ns*m+(idcol:idcol+3)-2) =[ exp(muSIn_m(IdIn-1)*(Xbdy(ii)-Xbdy(ii-1)))*((muSIn_m(IdIn-1)+dxmuS1In_m(IdIn-1)*(Xbdy(ii)-Xbdy(ii-1)))*IQnVpmIn(IdIn-1)+IdxVpmQnIn(IdIn-1)),  -muSIn_m(IdIn-1)*IQnVpmIn(IdIn-1)+IdxVpmQnIn(IdIn-1) , -muSIn_m(IdIn)*IQnVpmIn(IdIn)-IdxVpmQnIn(IdIn), -exp(-muSIn_m(IdIn)*(Xbdy(ii)-Xbdy(ii+1)))*((-muSIn_m(IdIn)-dxmuS1In_m(IdIn)*(Xbdy(ii)-Xbdy(ii+1)))*IQnVpmIn(IdIn)+IdxVpmQnIn(IdIn))];
                else
                    Amat(Nm*Ns*n+ii-idS1,Nm*Ns*m+(idcol:idcol+3)-2) =[exp(muSIn_m(IdIn)*(Xbdy(ii)-Xbdy(ii-1)))*IVpmVpnIn(IdIn), IVpmVpnIn(IdIn), -IVpmVpnIn(IdIn)    , -exp(-muSIn_m(IdIn)*(Xbdy(ii)-Xbdy(ii+1)))*IVpmVpnIn(IdIn)];
    ...
    if n==0
    Bvect(Nm*Ns*n+1)=-phiWl(1).*IQ0Vpn1L;
    Bvect(Nm*Ns*n+Nm/2)=phiWl(2).*IQ0Vpn1R;
    else
    Bvect(Nm*Ns*n+1-2)=-phiWl(1).*IQ0Vpn1L;
    Bvect(Nm*Ns*n+Nm/2-2)=phiWl(2).*IQ0Vpn1R;
    Bvect(Nm*Ns*n+Nm/2-2+1)=-(dxphiWl(1).*IQ0Qn1L+phiWl(1)*IdxQ0Qn1L);
    Bvect(Nm*Ns*n+Nm-2)=(dxphiWl(2).*IQ0Qn1R+phiWl(2)*IdxQ0Qn1R);
    end
    
%     assignin('base','AmatI',Amat);
%     assignin('base','BvectI',Bvect);
%     %      pause;
