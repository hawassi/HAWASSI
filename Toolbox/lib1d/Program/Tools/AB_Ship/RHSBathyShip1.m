...
function dz = RHSBathyShip1(time,z,model,par,influx,IVP,Oprt,shippar,ProgStatbar)
plot_ode='ye';
%%%%%passing parameter%%%%%%%%%%%%%%
...
dteta_hat       =  LphiF_hat;%LphiF_hat;
dtphi_hat       =  -fft(Ifft(deletaH_hat).*chifluid-dphidtS.*(1-chifluid)).*aal ;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if influx.wind.check==1
    Rw_hat=influx.wind.coef.*u_hat./(1i.*k);
else
    Rw_hat=0;
end
...
%         Zeta(ii,:)   =shippar.form.shapeXcZc(ii,:).*chiship(:,ii).';
        ...
        Dship=-par.bathy(closest(x,xShip(ii,2)));
        GM=shippar.form.GMperDship*Dship;
