...
%%%%%%%%%%%%%%%%%  RHS for RunUp  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dz = RHSShoreShip1phi(time,z,model,par,influx,IVP,Oprt,shippar,ProgStatbar)
%%%%%passing parameter%%%%%%%%%%%%%%
...
%%%%%ship part%%%%%%%%%%%%%%%%%%%%%%%%
  if shippar.calclin==1
       ...
       sZInDif=sZ;
        ...
        influx.k_p,-par.bathy',Nship,Evmode,kappa,time,Amat,Bvect,Smotion,nutild,shippar.form.XZrot0(1,:));
...
if model.nonlinear==2
    ...
    CCu_hat= Op_C2r(C2m1,gm1,C2p1,gp1,C2c1,gc1,...
                   C2m2,gm2,C2p2,gp2,C2c2,gc2,u,uhat);
    ...
    FluxF=deluH_hat;
    Lphif_hat=-1i.*k.*FluxF;  
    dteta_hat =   fft(Ifft((Lphif_hat+fft(Lev)+fft((FluxF+FEv-Fs-FpsiS).*dxchiship(:,end))).*aal).*chifluid+dtetaS);
    dtphi_hat   = -fft(Ifft(deletaH_hat.*1).*chifluid-dphidtS.*(1-chifluid)).*aal ;
