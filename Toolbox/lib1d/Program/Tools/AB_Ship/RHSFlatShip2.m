...
%%%%%%%%%%%%%%%%%  RHS for Flat-Bottom %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [dz] = RHSFlatShip2(time,z,model,par,influx,IVP,Oprt,shippar,ProgStatbar)
...
if shippar.calclin==0
    ...
    Oprt.gammin=ppval(Oprt.Sp_gammin,HtotS);
    Oprt.gamplus=ppval(Oprt.Sp_gamplus,HtotS);
    Oprt.gammid=ppval(Oprt.Sp_gammid,HtotS);
...
if strcmp(model.evol,'HS')
   ...
   if Nship==1
        ...
        -par.bathy,shippar.Evmode,shippar.form.XZrot0,influx.lambda_p,shippar.form.kappa,shippar.tangenflowGlobal,Oprt.L0,Oprt.F0,model.nonlinear,...
        ...
        if any(~strcmp(Smotion(:),'Fixed'))
            if Nship==1
              [shippar.rad.psi,shippar.rad.Ma,shippar.rad.dampC,shippar.rad.LpsiEv,...
                     shippar.rad.Fpsi,shippar.rad.FpsiEv,dxi_Krad]=fun_radiation_potential_anzats_Interp(g,k,par.x.',Oprt,shippar.tangenflowGlobal,shippar.form.shapeXcZc(end,:).',shippar.form.dxshapeXcZc(end,:).',Smotion,2,nutild,xShip,shippar.form.XZrot0,chiSWl(:,end),1,0,'Flat');
...
%                 chiwaterline,nu,Smotion,-par.bathy',Nship,0,0,0,0);   
            ...
            dxi_Krad.z=zeros(Nship,1);dxi_Krad.x=zeros(Nship,1);dxi_Krad.theta=zeros(Nship,1);
         end
    else
        dxi_Krad.z=zeros(Nship,1);dxi_Krad.x=zeros(Nship,1);dxi_Krad.theta=zeros(Nship,1);
    end
  ...
  %  dphidtS=((phitot-phiprev)/par.dt);
    ...
    FluxF  =ifft(funOprt_Flux(g,k,Oprt.Upd,eta,phi,model.nonlinear),'symmetric');
%     Fs  =ifft(funOprt_Flux(g,k,Oprt.Upd,eta,phif,model.nonlinear),'symmetric');
    
    Lphif_hat=funOprt_L(k,L0,eta,phi,model.nonlinear,ChiAdj);
...
if  Nship==2 && Iddamplid==1 
   ...
   % Ltau_hat=funOprt_L(k,L0,eta,tau,model.nonlinear,ChiAdj);
...
%     Lphif_hat=funOprt_L(k,L0,eta,phi.*chifluid,model.nonlinear,ChiAdj);
%     LphiF_hat=fft(Ifft((Lphif_hat+Ltau_hat).*aal)+dtetaS);
     
    dteta_hat     =   LphiF_hat; 
    dtphi_hat     = -deletaH_hat;%fft(Ifft(deletaH_hat).*chifluid-dphidtS.*chishipF).*aal ; 
end
...
etaVirt=smooth(Ifft(1i.*Oprt.Omd.*fft(phitot))/g,5);
...
%         Zeta(ii,:)   =shippar.form.shapeXcZc(ii,:).*chiship(:,ii).';
        ...
        if shippar.form.Cd.Theta(1)>0
            ...
            dtPz(ii,1)=dtPthe(ii,1)-Cd*Sdraft*dtThe*abs(dtThe)/2;
        end
...
if strcmp(plot_ode,'yes')
     if time> influx.input.t_init+ITERdt*par.dt && time>0
        if ITERdt==1, fig2=figure;end
        subplot(3,1,1)
        plot(x,eta,'r',x,phif,'b');xlim([-100 100]);
        title(['t= ',num2str(time)])
        subplot(3,1,2)
        plot(x,FluxF,'r',x,FEv,'b',x,Fs,'k');xlim([-100 100]);
        subplot(3,1,3)
        plot(x,Ifft(Lphif_hat),'r',x,Ifft(Lphif_hat),'b');xlim([-100 100]);
