...
%%%%%%%%%%%%%%%%%  RHS for Flat-Bottom %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [dz] = RHSFlatShip1(time,z,model,par,influx,IVP,Oprt,shippar,ProgStatbar)
...
if  Nship==2 && Iddamplid==1 
   ...
   %  Lphif_hat=fft(Ifft(Lphif_hat)+Lev.*chifluid);
    ...
    Lphif_hat=fft((Ifft(Lphif_hat)+Lev).*chifluid);
    LphiF_hat=(Lphif_hat-fft(sigma.*eta)+fft((FluxF+FEv-Fs-FpsiS).*dxchiship(:,end))).*aal;
...
dteta_hat   = dteta_hat + (Source  + Swall_hat - fft(par.dampcoef.*eta.*dampchar)).*aal;
...
if ~any(strcmp(Smotion,'Fixed')) && strcmpi(shippar.moored.check,'yes')
    ...
    wn2=(2*pi./Tn).^2;
    wn2(Tn==0)=0;
    dtPimoored=((shippar.form.MassMat+shippar.rad.Ma.Mat)*xi).*wn2;
end
