...
for i=1:Nship
    ...
    if strcmpi(shippar.data(i,2),'Pitch')||strcmpi(shippar.data(i,2),'Free') ||strcmpi(shippar.data(i,2),'Fixed')
        ...
        try
            Zrot0(i)=str2num(cell2mat(shippar.data(i,8)));
        catch
            Zrot0(i)=cell2mat(shippar.data(i,8));
        end
        try
    ...
    SXc=Xc(i);
    SZc=Zc(i);
    [shapeXcZcI,dxshapeXcZcO,shapeXcZcdati]=shapeformLib(x,SXc,SZc,Sdraft(i),Slength(i)/2,[],shipform);
    shapeXcZ0I=zeros(size(shapeXcZcI));
    shapeXcZ0I(shapeXcZcI<0)=shapeXcZcI(shapeXcZcI<0)-SZc;
    %     shapeX0Z0(i,:) = shapeOrigin';
%     IndSXC         =closest(x,SXc)-closest(x,0);
%     shapeXcZ0(i,:) = circshift(shapeOrigin',[IndSXC,0]);
    ...
    dxChi(i,indxR)=-1/dx;
%     if i==2
%         dxChi(i,indxL)=0;%dxChi(i,indxL).*0.5;
%     end
%     if i==1
%         dxChi(i,indxR)=0;%dxChi(i,indxR).*0.5;
%     end
%     
    ...
    shapeN=shapeXcZc(i,:);shapeN([indxL indxR])=-Sdraft(i);
