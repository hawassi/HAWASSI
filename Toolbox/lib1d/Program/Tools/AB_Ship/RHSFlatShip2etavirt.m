...
%%%%%%%%%%%%%%%%%  RHS for Flat-Bottom %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [dz] = RHSFlatShip2etavirt(time,z,model,par,influx,IVP,Oprt,shippar,ProgStatbar)
        ...
        if any(~strcmp(Smotion(:),'Fixed'))
            if Nship==1
                [shippar.rad.psi,shippar.rad.Ma,shippar.rad.dampC,shippar.rad.LpsiEv,...
                    shippar.rad.Fpsi,shippar.rad.FpsiEv,dxi_Krad]=fun_radiation_potential_anzats_Interp(g,k,par.x.',Oprt.S,shippar.tangenflowGlobal,shippar.form.shapeXcZc(end,:).',shippar.form.dxshapeXcZc(end,:).',Smotion,2,nutild,xShip,shippar.form.XZrot0,chiSWl(:,end),1,0,'F');
...
%                 chiwaterline,nu,Smotion,-par.bathy',Nship,0,0,0,0);   
    ...
    phitot=phif+phiRad;
     ...
     %% if lambda peak>>>
    if influx.lambda_p>10*Slength(1)
    D=-par.bathy;
    FluxF  =Fs.*D./(D-max(shippar.form.shapedat(1).dr_approx));%
    else
...
if any(strcmp(Smotion,'Surge'))||any(strcmp(Smotion,'Free'))
    ...
    for ii=1:Nship
        ...
        Idlindamp=0;
        if Idlindamp==1
            disp('warning damp coef');
            % dampCoef=0.0019354; %%for scaling 100 rho=1 barge wang
            dampCoef=0.019354; %% for rho=1 scaling 1 
            dtPx(ii,1)=dtPx(ii,1)-dampCoef*dtsX;
        end
