function [shape,dxshape,shapedat]=shapeformLib(x,Sxc,Szc,T,ell,betaL,shipform)
draft=T-Szc;
shapedat=0;
switch shipform
    case 'cfGauss'
    ...
    case 'Barge'
        ...
        shapedat=[sort(XSL.') sort(ZSL.','descend' ); XSR(2:end).' ZSR(2:end).'];
        
        shape=zeros(size(x));
        indx1=closest(x,min(shapedat(:,1)));
        indx2=closest(x,max(shapedat(:,1)));
        if x(indx1)<shape(1,1)
            indx1=indx1+1;
        end
        
        if x(indx2)>shape(end,1)
            indx2=indx2-1;
        end
        ...
        dxshape = DDR-DDL;dxshapeGrad=gradient(shape,dx);
%                 figure
%                 plot(x,shape,'r',x,dxshape,'b',x,dxshapeGrad,'k')
    ...
    case 'Wedge'
        ...
        shapedat=[sort(X_min.') sort(zT0min.','descend' ); X_plus(2:end).' zT0plus(2:end).'];
        shape=zeros(size(x));
