function plotspec(S,axesfig,varargin)
  ...
  case {'dir'}
    ...
    if plotflag==1 % polar plot
      if 0, % alternative but then z_level must be chosen beforehand
        ...
        c = contour(axesfig,theta,freq,S.S');%,Nlevel); % calculate levels
        if isempty(c)
          c = contour (theta,freq,S.S);%,Nlevel); % calculate levels
        end
        [z_level c] = clevels(c); % find contour levels
        h = polar(axesfig,c(1,:),c(2,:));
        rotate(h,[0 0 1],-phi*180/pi)
         colormap(axesfig,'jet')
       %  cb=colorbar('eastoutside');
      end
      title(axesfig,ylbl3_txt)
      % label the contour levels
