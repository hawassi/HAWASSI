function funP_anim_density(simuldata,setting,axesfig,handles)
hfig=handles.fig;
varId=setting.var;
...
if setting.saveanim.check==1
 ...
 %rect=[1.71*marg, 0.2*marg, pos(3)+4*marg, pos(4)+1.5*marg];
...
%     end
    ...
    stringtitle=['Evolution of ',varname,' @ time: ',num2str(roundn(T(indt),-2)), ' [s]'];
...
%         end
        funP_contour(axesfig,XX,YY,varlevel,thresholdLevel,max(max(var_xy)),'k',1.5);
        hold(axesfig,'off');
    end
    ...
    if ~isempty(output.break_nodes)
       ...
       if saveanim==1
           ...
           filename=[Proj.workdir,'PP_anim_density_',varname,'.gif'];
           if ii == 1
    ...
    if get(handles.pp_anim_pause,'userdata')==1
     uiwait(gcf);   
    end
    
    if get(handles.pp_anim_stop,'userdata')==1
         set(handles.pp_anim_stop,'userdata',0);
        break;        
    end
 end
