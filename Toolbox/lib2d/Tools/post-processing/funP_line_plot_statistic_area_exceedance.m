function funP_line_plot_statistic_area_exceedance(simuldata,spatsnap,setting,axesfig,h)
...
xlab='\eta_{\rm crest}/H_s [-]';
etarefplus=linspace(0,max(max(max(varP(indt1:indt2,indy1:indy2,indx1:indx2)))),100);
etarefmin=linspace(0,max(max(max(-varP(indt1:indt2,indy1:indy2,indx1:indx2)))),100);
...
tic;
iter=1;
Exceedplus=zeros(Ntt,length(etarefplus));
Exceedmin=zeros(Ntt,length(etarefmin));
...
varTiplus=squeeze(varP(indtti,indy1:indy2,indx1:indx2));
varTimin=squeeze(-varP(indtti,indy1:indy2,indx1:indx2));

for jj=1:length(etarefplus)
        Exceedplus(ii,jj)=length(varTiplus(varTiplus>etarefplus(jj))) ;    
end
for jj=1:length(etarefmin)
        Exceedmin(ii,jj)=length(varTimin(varTimin>etarefmin(jj))) ;    
end
...
Prob_Areaexceedplus=sum(Exceedplus)./Ntotal;
Prob_Areaexceedmin=sum(Exceedmin)./Ntotal;
...
semilogy(axesfig,etarefplus/Hs,Prob_Areaexceedplus,'or',etarefmin/Hs,Prob_Areaexceedmin,'*b')   
xlabel(axesfig,xlab)
...
% xlim(axesfig,[etaref(1) etaref(end)]);
 ...
 if setting.savedat==1
        varr=[(etarefplus/Hs)' Prob_Areaexceedplus' (etarefmin/Hs)' Prob_Areaexceedmin'];
