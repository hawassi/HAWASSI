function [InterpDisp]= funOprt_DispInterpolation_2p(g,dom,Up_fun,Om_fun,omAdd,nupeak,Depth,bath,...
                            Proj,dynmodel)
...
for j= 1:length(DDepth)
    D               = DDepth(j);
    kappanu_D(j)    = funOprt_invOmExactVal(nu,D,g);%interp1(Om_fun(k,D,g),k,nu,'spline');%interpolation to find k at peak frequency nu
