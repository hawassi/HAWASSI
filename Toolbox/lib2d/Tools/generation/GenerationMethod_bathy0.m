function [Gen] = GenerationMethod_bathy0(g,Infmodel,dom,Om,Ug,ugadd,depthInfl,...
...
if strcmp(Infmodel.type(I),'Point')
    ...
    if strcmp(Spatinflux.line(I).Orientation,'Vertical')
        ...
        dxGam(indyinf,:)=funC_ifft(1i.*Om(kk,depthInfl(jj),g,ugadd).*fft(deltadir)); %for uni-direction
        end
       ...
       % dyGam(:,indXinf)=funC_ifft(1i.*kk'.*fft(deltadir)); %for uni-direction
        dyGam(:,indXinf)=funC_ifft(1i.*Om(kk,depthInfl(jj),g,ugadd).*fft(deltadir)); %for uni-direction
        end
        Gam_hat =fft2(Gam.*Heav')*fact;
        Gam     = funC_ifft2(Gam_hat);
...
elseif strcmp(Infmodel.type(I),'Area')
    if strcmp(Spatinflux.line(I).Orientation,'Vertical')
       ...
       % dxGam(indyinf,:)=funC_ifft(1i.*kk'.*fft(Ugx)); %for uni-direction
        dxGam(indyinf,:)=funC_ifft(1i.*Om(kk,depthInfl(jj),g,ugadd).*fft(Ugx)); %for uni-direction
        end
        Gam_hat    = fft2(Gam.*Heav)*fact;
        Gam        = funC_ifft2(Gam_hat);
        dxGam_hat  =fft2(dxGam.*Heav)*fact;
       ...
       % dyGam(:,indxinf)=funC_ifft(1i.*kk'.*fft(Ugy)); %for uni-direction
        dyGam(:,indxinf)=funC_ifft(1i.*Om(kk,depthInfl(jj),g,ugadd).*fft(Ugy)); %for uni-direction
        end
        Gam_hat  = fft2(Gam.*Heav')*fact;
        Gam      = funC_ifft2(Gam_hat);
        dyGam_hat= fft2(dyGam.*Heav')*fact;
