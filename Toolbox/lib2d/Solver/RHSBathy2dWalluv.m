...
elseif strcmp(model.evol,'HS')
    if model.nonlinear==1
        ...
        M0uv          = funC_ifft2(M0uvwall_hat);%funOprt_M2d(g,dom.Kx,dom.Ky,Oprt.Cp2dSqInterp,u_hat,u,v_hat,v);
...
%         C2UV_hat.x_hat=funOprt_Cp2d(Oprt.Cp2dSqInterp,u_hat,u);
%         C2UV_hat.y_hat=funOprt_Cp2d(Oprt.Cp2dSqInterp,v_hat,v);
%         C2UV_wall_hat.x_hat=fft2(funC_ifft2(C2UV_hat.x_hat).*Wallchar);
%         C2UV_wall_hat.y_hat=fft2(funC_ifft2(C2UV_hat.y_hat).*Wallchar);
%         div_C2UVWall_hat=funOprt_div2d(dom.Kx,dom.Ky,C2UV_wall_hat);
        %deletaH2_hat  = 0.5*fft2(gradphi2.*Wallchar+M0uv.*funC_ifft2(div_C2UVWall_hat)./g);
