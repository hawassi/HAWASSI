...
function [dz] = RHSFlat2dWall(time,z_hat,input,model,par,parBreak,dom,influx,ivp,...
...
elseif strcmp(model.evol,'HS')
    ...
    if model.nonlinear==1
        ...
        Cu.x                 =funC_ifft2(Oprt.Cp2d.*fft2(gradphi.x));
        Cu.y                 =funC_ifft2(Oprt.Cp2d.*fft2(gradphi.y));
        Cp2gradphiWall_hat.x_hat = Oprt.Cp2d.*fft2(Cu.x.*Wallchar);
        Cp2gradphiWall_hat.y_hat = Oprt.Cp2d.*fft2(Cu.y.*Wallchar);
        
        Lphiwall_hat= -funOprt_div2d(dom.Kx,dom.Ky,Cp2gradphiWall_hat)/g;    
        ...
        Lphi              =funC_ifft2(Oprt.L2d.*phi_hat);
        LetaLphiWall_hat  = Oprt.L2d.*fft2(eta.*Lphi.*Wallchar);
        ...
        if model.nonlinear~=2
            LetaLphi        =funC_ifft2(Oprt.L2d.*fft2(eta.*Lphi));
            LetaWallLetaLphi_hat=Oprt.L2d.*fft2(eta.*Wallchar.*LetaLphi);
            LetaLetaWallLphi_hat=Oprt.L2d.*fft2(eta.*funC_ifft2(LetaLphiWall_hat));
...
if plotchecker==1
    ...
    if mod( iterdtcheck,2)
        % subplot(2,1,1)
        surf(X,Y,funC_ifft2(Lphiwall_hat),'Edgecolor','none')
        view(2)
        colorbar;
    %    caxis([-0.1;0.1]);
       % zlim([-0.1;0.1])
