...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [dz] = RHSShore2d(time,z_hat,input,model,bath,par,parBreak,dom,influx,ivp,...
...
% Wallchar=ones(size(eta));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Cp2dsq = Oprt.Cp2dSqInterp;
CharH  = ones(size(eta));
CharH(H<H_min)= 0;
Cp2dsq.gammin1  = ppval(Cp2dsq.sp_gammin1,H).*CharH; 
Cp2dsq.gamplus1 = ppval(Cp2dsq.sp_gamplus1,H).*CharH; 
Cp2dsq.gammid1  = ppval(Cp2dsq.sp_gammid1,H).*CharH; 
Cp2dsq.gammin2  = ppval(Cp2dsq.sp_gammin2,H).*CharH; 
Cp2dsq.gamplus2 = ppval(Cp2dsq.sp_gamplus2,H).*CharH; 
Cp2dsq.gammid2  = ppval(Cp2dsq.sp_gammid2,H).*CharH; 
...
deletaH_hat = fft2(g.*eta) + 1/2*fft2((gradphi2-Lphi.^2).*ChiAdj); 
...
if (damptype == 0)%original
    ...
    if strcmp(bath.type,'Shore (in x-axis)')
        ...
        dteta_hat=(delphiH_hat+Source_hat-fft2(Upeta.*dampchar)).*aal;
        %dtphi_hat=(-deletaH_hat+Sb_hat+Sf_hat-fft2(Updxphi.*dampchar)./(1i*dom.Kx)).*aal;
       %dtphi_hat=(-deletaH_hat+Sb_hat+Sf_hat-fft2(Upphi.*dampchar)+fft2(graddampchar.x.*Upphi)./(1i*dom.Kx) ).*aal;
...
%        corrdamp2_hat(:,1)=0;
        ...
        dtphi_hat=(-deletaH_hat+Sb_hat+Sf_hat-fft2(Upphi.*dampchar)+corrdamp1_hat).*aal;
    else
        Upeta=dampcoef*eta;
