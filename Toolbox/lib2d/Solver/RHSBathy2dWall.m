...
function [dz] = RHSBathy2dWall(time,z_hat,input,model,par,parBreak,dom,influx,ivp,...
...
if strcmp(model.evol,'SWE')
    ...
    if model.nonlinear==1
        ...
        Hgradphi_hat.x_hat=fft2((Depth+eta).*gradphi.x.*Wallchar);
        Hgradphi_hat.y_hat=fft2((Depth+eta).*gradphi.y.*Wallchar);
...
elseif strcmp(model.evol,'HS')
    ...
    if model.nonlinear==1
        if strcmp(dom.wall.ReflCoef.AllOneFlagCutK,'True')
            ...
            Lphiwall_hat= fft2(funC_ifft2(-funOprt_div2d(dom.Kx,dom.Ky,Cp2gradphi_hat)/g).*Wallchar)...
            ...
            Lphiwall_hat= -DivCp2gradphiWall_hat/g;
        end
        delphiH1_hat  = Lphiwall_hat;
        ...
        etaLphiWall   = eta.*Lphi.*Wallchar;
        LetaLphiWall_hat= funOprt_L2d(g,Oprt.Om2dSqInterp,fft2(etaLphiWall),etaLphiWall);
        gradphi       = funOprt_grad2d(dom.Kx,dom.Ky,phi_hat);
        ...
        gradphi2      = funOprt_innerproduct(gradphi,gradphi);
        delphiH2_hat  = -(LetaLphiWall_hat+divetagradphiWall_hat);
        deletaH2_hat  = 0.5*fft2((gradphi2-Lphi.^2).*Wallchar);
        if model.nonlinear==2
            delphiH3_hat  =0; deletaH3_hat=0;
        else
            
            
            LetaLphi_hat        =funOprt_L2d(g,Oprt.Om2dSqInterp,...
                fft2(eta.*Lphi),eta.*Lphi);
            LetaLphi           =funC_ifft2(LetaLphi_hat);
            etaWallLetaLphi     =eta.*Wallchar.*LetaLphi;
            LetaWallLetaLphi_hat=funOprt_L2d(g,Oprt.Om2dSqInterp,...
                fft2(etaWallLetaLphi),etaWallLetaLphi);
            LetaLetaWallLphi_hat=funOprt_L2d(g,Oprt.Om2dSqInterp,...
                fft2(eta.*funC_ifft2(LetaLphiWall_hat)),...
                eta.*funC_ifft2(LetaLphiWall_hat));
            
            Walleta2Lphi_hat=fft2(Wallchar.*eta.^2.*Lphi);
            gradWalleta2Lphi_hat=funOprt_grad2d_hat(dom.Kx,dom.Ky,Walleta2Lphi_hat);
            DivGradWalleta2Lphi_hat=funOprt_div2d(dom.Kx,dom.Ky,gradWalleta2Lphi_hat);
            Gradphi_hat    =funOprt_grad2d_hat(dom.Kx,dom.Ky,phi_hat);
            ...
            delphiH3_hat  =0.5*(LetaWallLetaLphi_hat+LetaLetaWallLphi_hat+...
                Leta2WallDivGradphi_hat+DivGradWalleta2Lphi_hat);
            deletaH3_hat  =fft2(Lphi.*(eta.*DivGradphi.*Wallchar+...
                0.5*funC_ifft2(LetaLphiWall_hat)+0.5*Wallchar.*LetaLphi));
            
            %             gradetaLphi=funOprt_grad2d(dom.Kx,dom.Ky,fft2(eta.*Lphi));
            %             CgradetaLphi_hat.x_hat= funOprt_Cp2d(Oprt.Cp2dInterp,fft2(gradetaLphi.x),gradetaLphi.x);
            %             CgradetaLphi_hat.y_hat= funOprt_Cp2d(Oprt.Cp2dInterp,fft2(gradetaLphi.y),gradetaLphi.y);
            %             WallCgradetaLphi.x=funC_ifft2(CgradetaLphi_hat.x_hat).*Wallchar;
            %             WallCgradetaLphi.y=funC_ifft2(CgradetaLphi_hat.y_hat).*Wallchar;
            %             CWallCegradtaLphi_hat.x_hat=funOprt_Cp2d(Oprt.Cp2dInterp,fft2(WallCgradetaLphi.x),WallCgradetaLphi.x);
            %             CWallCegradtaLphi_hat.y_hat=funOprt_Cp2d(Oprt.Cp2dInterp,fft2(WallCgradetaLphi.y),WallCgradetaLphi.y);
            %             DivCWallCgradetaLphi_hat=funOprt_div2d(dom.Kx,dom.Ky,CWallCegradtaLphi_hat);
            %             etaDivCWallCgradetaLphi=eta.*funC_ifft2(DivCWallCgradetaLphi_hat);
            %             LetaDivCWallCgradetaLphi_hat=funOprt_L2d(g,Oprt.Om2dSqInterp,fft2(etaDivCWallCgradetaLphi),etaDivCWallCgradetaLphi);
            %
            %             eta2DivCp2gradphiWall=eta.^2.*funC_ifft2(DivCp2gradphiWall_hat);
            %             gradeta2DivCp2gradphiWall_hat=funOprt_grad2d_hat(dom.Kx,dom.Ky,fft2(eta2DivCp2gradphiWall));
            %
            %             eta2gradphi.x=eta.^2.*gradphi.x;eta2gradphi.y=eta.^2.*gradphi.y;
            %             Ceta2gradphi_hat.x_hat=funOprt_Cp2d(Oprt.Cp2dInterp,fft2(eta2gradphi.x),eta2gradphi.x);
            %             Ceta2gradphi_hat.y_hat=funOprt_Cp2d(Oprt.Cp2dInterp,fft2(eta2gradphi.y),eta2gradphi.y);
            %             WallCeta2gradphi.x=Wallchar.*funC_ifft2(Ceta2gradphi_hat.x_hat);
            %             WallCeta2gradphi.y=Wallchar.*funC_ifft2(Ceta2gradphi_hat.y_hat);
            %             CWallCeta2gradphi_hat.x_hat=funOprt_Cp2d(Oprt.Cp2dInterp,fft2(WallCeta2gradphi.x),WallCeta2gradphi.x);
            %             CWallCeta2gradphi_hat.y_hat=funOprt_Cp2d(Oprt.Cp2dInterp,fft2(WallCeta2gradphi.y),WallCeta2gradphi.y);
            %
            %             Divgradeta2DivCp2gradphiWall_hat=funOprt_div2d(dom.Kx,dom.Ky,gradeta2DivCp2gradphiWall_hat);
            %             DivCWallCeta2gradphi_hat=funOprt_div2d(dom.Kx,dom.Ky,CWallCeta2gradphi_hat);
            %
            %             LphiDivCWallCgradetaLphi=funC_ifft2(DivCWallCgradetaLphi_hat).*Lphi;
            %             gradphi_hat       = funOprt_grad2d_hat(dom.Kx,dom.Ky,phi_hat);
            %             Divgradphi_hat=funOprt_div2d(dom.Kx,dom.Ky,gradphi_hat);
            %             DivCp2gradphiWall_etaDivgradphi=funC_ifft2(DivCp2gradphiWall_hat).*eta.*funC_ifft2(Divgradphi_hat);
            %
            %             delphiH3_hat  =-(LetaDivCWallCgradetaLphi_hat+...
            %                 0.5*(Divgradeta2DivCp2gradphiWall_hat+DivCWallCeta2gradphi_hat))/g;
            %             deletaH3_hat  =-fft2(LphiDivCWallCgradetaLphi+DivCp2gradphiWall_etaDivgradphi)./g;
        end
    end
...
dteta_hat=(delphiH_hat+ Swall_hat  +Source_hat-dampcoef*fft2(eta.*dampchar)).*aal;
